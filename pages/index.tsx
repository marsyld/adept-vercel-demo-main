...
function FragmentedTab({ text, img }: { text: string; img: string }) {
  return (
    <>
      <motion.div
        className="rounded-2xl p-6 bg-white/[0.06] border border-white/10 h-full flex items-center"
      ...
      <motion.div
        className="rounded-2xl overflow-hidden border border-white/10 bg-white/5 flex justify-center items-center"
        initial={{ opacity: 0, x: 10 }}
        animate={{ opacity: 1, x: 0 }}
        exit={{ opacity: 0, x: 10 }}
        transition={{ duration: 0.4 }}
...
function FaqRow({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="bg-white/[0.02]">
      ...
      {open && <div className="px-5 pb-5 pt-0 text-white/75">{a}</div>}
    </div>
  );
}
